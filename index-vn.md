# 🐍 DSAI1003 – Các nguyên lý lập trình cơ bản với Python

🌐 **Ngôn ngữ:** 🇻🇳 **Tiếng Việt** | [🇬🇧 English Version (index-en.md)](index-en.md)

> **Giảng viên:** TS. Vũ Đức Minh  
> **Đơn vị phụ trách:** Khoa Khoa học dữ liệu & Trí tuệ nhân tạo, Trường Công nghệ, Đại học Kinh tế Quốc dân (NEU)  
> **Chương trình:** Khoa học dữ liệu trong Tài chính và Thương mại điện tử (DSFE)  
> **Số tín chỉ:** 3 Tín chỉ (45h lý thuyết + 22.5h thực hành (option), 90h tự học)  
> **Đề cương chi tiết:** Xem tệp [syllabus-vn.md](syllabus-vn.md) | [syllabus-en.md](syllabus-en.md)

---

## 🚀 Chuyên đề Phương pháp Học tập

| Tài liệu / Chuyên đề | Định dạng / Liên kết |
| :--- | :---: |
| 📘 **Cẩm nang Học phần & Hướng dẫn Tự học** (Handbook A4, 11 trang) | [**Tải PDF**](lectures/part00-starting-points/python_course_guide.pdf ':ignore target=_blank') |
| 🖥️ **Slide Giới thiệu & Định hướng Môn học** (Beamer 16:9, 26 slides) | [**Tải Slide PDF**](lectures/part00-starting-points/python_course_guide_slides.pdf ':ignore target=_blank') |
| 📖 **Phương pháp Tự học Lập trình Python Hiệu quả** (Tutorial/Lab) | [**Xem tài liệu**](lectures/part00-starting-points/tutorial-effective-python-self-study-vn.md) |
| 🤖 **Chiến lược Sử dụng Trí tuệ Nhân tạo (ChatGPT/AI) Hỗ trợ Học tập** (Tutorial/Lab) | [**Xem tài liệu**](lectures/part00-starting-points/learning-python-with-AI-tool-vn.md) |

---

## 📌 1. Giới thiệu Học phần & Mục tiêu

Học phần **Các nguyên lý lập trình cơ bản với Python (DSAI1003)** trang bị cho sinh viên nền tảng tư duy lập trình máy tính có hệ thống và kỹ năng giải quyết bài toán thông qua ngôn ngữ Python.

### 🎯 Mục tiêu học phần (Course Objectives):
- **G1:** Xây dựng chương trình dạng console và giao diện cơ bản giải quyết bài toán thực tế trong kinh doanh.
- **G2:** Vận dụng thành thạo các cấu trúc điều khiển rẽ nhánh (`if-elif-else`) và vòng lặp (`for`, `while`).
- **G3:** Thiết kế các hàm module hóa và hàm đệ quy phục vụ tái sử dụng mã nguồn.
- **G4:** Thao tác hiệu quả trên các cấu trúc dữ liệu tích hợp (List, Tuple, Set, Dictionary).
- **G5:** Đọc/ghi tệp tin văn bản, tệp nhị phân và thiết lập cơ chế xử lý ngoại lệ an toàn (`try-except`).
- **G6:** Nắm vững các nguyên lý lập trình hướng đối tượng cơ bản (Lớp, Đối tượng, Kế thừa, Đa hình).
- **G7:** Nâng cao năng lực tự học, giải quyết vấn đề độc lập và làm việc nhóm hiệu quả.

---

## 📚 2. Ma trận Bài giảng, Tài liệu & Bài tập Thực hành

| Phần / Chủ đề | Nội dung bài học | Bài giảng & Bài đọc (.md / .ipynb) | Slide | Bài tập Lab | Đáp án | Trạng thái |
|:---:|:---|:---|:---:|:---:|:---:|:---:|
| **Part 01** | **Bài 1: Giới thiệu chung & Thiết kế giải thuật** | • [Bài đọc 1: Giới thiệu lập trình & Python](lectures/part01-introduction/introduction-vn.md)<br>• [Bài giảng: Nhập/Xuất cơ bản](lectures/part01-introduction/simple_input_output-vn.md) | • [Slide Tổng quan (PDF)](lectures/part01-introduction/part01-introduction-vn.pdf ':ignore target=_blank')<br>• [Slide Nhập/Xuất (PDF)](lectures/part01-introduction/simple_input_output_slide-vn.pdf ':ignore target=_blank') | • Setup môi trường<br>• [Lab 1: Nhập/Xuất cơ bản (.ipynb)](lectures/part01-introduction/simple_input_output_lab-vn.ipynb ':ignore target=_blank') | - | ✅ *Đã sẵn sàng* |
| **Part 02** | **Bài 2: Làm việc với Số và Chuỗi ký tự** | • [Bài giảng 2: Số, Chuỗi & Toán tử (PDF)](lectures/part02-number-string-operators/part02-number-string-operators-en.pdf ':ignore target=_blank')<br>• [Bài đọc: Số, Chuỗi & Toán tử (.md)](lectures/part02-number-string-operators/part02-number-string-operators-vn.md) | • [Slide: Số & Chuỗi (PDF)](lectures/part02-number-string-operators/part02-number-string-operators-slide-vn.pdf ':ignore target=_blank') | • [Bài tập: Số & Chuỗi (.ipynb)](lectures/part02-number-string-operators/part02-programming-exercise-en.ipynb ':ignore target=_blank') | - | ✅ *Đã sẵn sàng* |
| **Part 03** | **Bài 3: Cấu trúc ra quyết định (Decision Making)** | - | - | - | - | ⏳ *Đang biên soạn* |
| **Part 04** | **Bài 4: Cấu trúc vòng lặp (Loops)** | - | - | - | - | ⏳ *Đang biên soạn* |
| **Part 05** | **Bài 5: Thiết kế hàm & Phạm vi biến (Functions)** | - | - | - | - | ⏳ *Đang biên soạn* |
| **Part 06** | **Bài 6: Cấu trúc List và Tuple** | - | - | - | - | ⏳ *Đang biên soạn* |
| **Đánh giá** | **Thi giữa kỳ (Midterm Exam - Nội dung Part 01 – 06)** | - | - | - | - | ⏳ *Đang biên soạn* |
| **Part 07** | **Bài 7: Thao tác Tệp và Xử lý Ngoại lệ (Files & Exceptions)** | - | - | - | - | ⏳ *Đang biên soạn* |
| **Part 08** | **Bài 8: Tập hợp (Set) và Từ điển (Dictionary)** | - | - | - | - | ⏳ *Đang biên soạn* |
| **Part 09** | **Bài 9: Đối tượng và Lớp (OOP - Classes & Objects)** | - | - | - | - | ⏳ *Đang biên soạn* |
| **Part 10** | **Bài 10: Kế thừa và Đa hình (Inheritance & Polymorphism)** | - | - | - | - | ⏳ *Đang biên soạn* |
| **Tổng kết** | **Hệ thống hóa kiến thức & Ôn tập cuối kỳ** | - | - | - | - | ⏳ *Đang biên soạn* |

---

## 📖 3. Sách tham khảo (References & Textbooks)

| Bìa sách | Tài liệu | Tác giả | Nhà xuất bản | ISBN | Liên kết |
|:---:|:---|:---|:---|:---:|:---:|
| <img src="assets/images/python-for-everyone-3rd-cover.jpg" alt="Python for Everyone 3rd Edition" width="90" /> | **[1] Python for Everyone**, 3rd Edition | Cay Horstmann, Rance Necaise | John Wiley & Sons, 2019 | 9781119498537 | [Thông tin sách](https://www.wiley.com/en-us/Python+for+Everyone%2C+3rd+Edition-p-9781119498537) |
| <img src="assets/images/intro-to-python-deitel-cover.jpg" alt="Intro to Python for Computer Science and Data Science" width="90" /> | **[2] Intro to Python for Computer Science and Data Science: Learning to Program with AI, Big Data and The Cloud** | Paul Deitel, Harvey Deitel | Pearson Education, 2020 | 9780135404676 | [Thông tin sách](https://www.pearson.com/en-us/subject-catalog/p/intro-to-python-for-computer-science-and-data-science-learning-to-program-with-ai-big-data-and-the-cloud/P200000003504/9780135404676) |
| <img src="assets/images/the-python-workbook-cover.jpg" alt="The Python Workbook" width="90" /> | **[3] The Python Workbook: A Brief Introduction with Exercises and Solutions**, 2nd Edition | Ben Stephenson | Springer, 2019 | 9783030188733 | [Thông tin sách](https://link.springer.com/book/10.1007/978-3-030-18873-3) |

---

> © 2026 TS. Vũ Đức Minh – Đại học Kinh tế Quốc dân (NEU). Bản quyền tài liệu thuộc về tác giả.
